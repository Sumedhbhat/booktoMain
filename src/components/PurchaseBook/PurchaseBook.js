...
import { purchaseNewBook } from "../redux/actions";
...
function PurchaseBook() {
  ...
  useEffect(() => {
    API.get("booksapi", "/books/booksId").then((res) => {
      setBookData([...bookData, ...res]);
    });
  }, []);
  console.log(bookData);
  const { id } = useParams();
  let book = bookData.filter((d) => d.booksId === id);
  ...
  const purchaseBook = async (
    ...
    await bookContract.methods
      ...
      .then((res) => {
        ...
        dispatch(purchaseNewBook(API, booksData, setShowDownloadBtn));
      })
                  ...
                  <div key={key}>
                    <div className='row purchaseBookRow z-depth-3'>
                      ...
                      <div className='col l12 s12'>
                        ...
                        >
                          <button className='btn purchaseBookPurchaseBtn'>
                            Purchase
                          </button>
                        </div>
                      </div>
                      {showDownloadBtn && (
                        <div className='col l12 s12'>
                          <div className='center-align'>
                            <p className='white-text purchaseBookTitles'>
                              Click on below button to access your purchased
                              book
                            </p>
                            <a
                              href={data.content}
                              target='_blank'
                              className='waves-effect btn purchaseBookDownloadBtn'
                            >
                              Download{" "}
